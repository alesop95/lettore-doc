...
Prossima azione: **nessun debito aperto** sul lato `ONEDRIVE`. Per un nuovo ciclo partire da `.\scripts\session_resume.ps1`. Candidate mai ingerite su quel lato, per dimensione: `_DA SISTEMARE (Alessio)` (44), un blocco coeso dei piccoli `Helpdesk_*` (NinjaOne, Microsoft 365, Onboarding, Amministrazione, INFOCERT, ABBYY, Timbracartellini, circa 35 in totale), `TOOL AI coding assistance` (9). Le grandi (`ENIVIPA` 2500, `SCENIA` 918, `OpenAI` 267) restano da affrontare con una segmentazione per coesione semantica, non in blocco.

Sorgente nuova disponibile dal 2026-09-14, mai ingerita fino a oggi: `LETTERDOC_SOURCE_PORTFOLIO`, riparato e ripuntato su Proton (dettaglio nel work-log). Copre `IT-RELATED`, `Progetti (consulenza)`, `Progetti (idee)` e `🛠️ Ongoing studies`, 1004 file in cinque aree (Cybersec/BC/governance, Generative AI, Quantitative Finance, Full-stack development, qbit computing).

**Primo ciclo aperto lo stesso giorno**: `OngoingStudies-Certs`, 19 documenti da `CERTs` (CISSP, CompTIA Security+, ISO/IEC 27001, certificazioni abbandonate). Passo 0 chiuso: copiati e verificati per SHA256, tutti passano il filtro sui nomi, cartella `-sanitized/` generata. **Prossima azione concreta su questo filone**: Passo 1, la sessione interattiva `/graphify .` sotto l'account dedicato, `.\scripts\start_graphify.ps1 -SourceFolder "_intermediate\src\OngoingStudies-Certs-sanitized" -Account account2`. Dettaglio nel work-log del 2026-09-14.
