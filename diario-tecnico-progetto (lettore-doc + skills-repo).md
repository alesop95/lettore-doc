...
## C.18 Ciclo Helpdesk T-Rex, e i falsi nomi propri del riconoscitore italiano (2026-08-05)

Il ciclo su Helpdesk_T-Rex era il primo dopo la riapertura, con nessun debito aperto ereditato da C.17. La cartella sorgente su OneDrive contiene quarantuno documenti processabili, mescolati a molto materiale non tecnico: presentazioni verso i clienti Angelini, EniVIPA e FAE, catene di email .eml sui singoli ticket, screenshot di fatture, note di credito, ordini di acquisto, file con nomi propri di persona come NASTASI LUIGI DOMENICO_..., Quartara Ts.docx e Autorizzazioni utente Roberta Vergani. La regola documentata su questo passo dice di escludere il materiale contrattuale e commerciale, i dati di terzi, e i documenti il cui nome contiene un dato personale, che va neutralizzato all'ingresso: la selezione manuale ha portato a ventiquattro documenti su quarantuno, in prevalenza documentazione tecnica pura del gestionale T-Rex, procedure di magazzino, cambio anno delle sequenze di fatturazione, tour registrati del prodotto, note sull'upgrade GroupShare e appunti sulla configurazione IMAP di Exchange Online. Il file 102025 - Note migrazione gestionale (call con Favale).txt e' stato rinominato in 102025 - Note migrazione gestionale.txt alla copia, per togliere il nome della persona dal nome file prima ancora del pre-flight.

Il passo 0 di prepare_graphify_source.py ha segnalato un unico file da neutralizzare per il nome, Problema CSRF Token T_Rex.docx, che il filtro di graphify scarterebbe per la parola token. La mappa di sostituzione ha reso il nome in Problema CSRF chiave T_Rex.md, che e' descrittivo e passa il filtro. Poi /graphify sulla cartella _intermediate\src\Helpdesk-T-Rex-sanitized\ ha prodotto un grafo di centodiciotto nodi e centotrentaquattro archi in tredici community. Il benchmark di token stimava una riduzione di 89.7 volte rispetto a passare il corpus intero come contesto.

La classificazione contro la tassonomia ha dato trentuno fit iniziali, di cui quindici marcati DA VERIFICARE per margine zero sul secondo classificato o per decisione presa su un solo token. Il gate sanitize_taxonomy_diff.py ne ha scartati cinque per label troppo scarno dopo l'anonimizzazione, come CRM module e Team PM, o per residuo di dominio di terzi come SMTP Exchange (smtp.outlook.com).

La revisione manuale del diff ha rifiutato altri undici fit chiaramente sbagliati e la sola nuova Capability suggerita. Il pattern degli errori era netto: la Capability Time & Attendance Management aveva attirato otto nodi che non c'entrano niente con l'omologa area di IT Operations, cioe' Trados Studio 2026 Release, GroupShare 2020 SR1 e SR2+CU15, MultiTerm Server API REST v2, ESXi snapshot pre-upgrade, Backup SQL Server, RWS Support e Pacchetti .sdlppx/.sdlrpx. La radice e' che quella pagina ha nel proprio ## Technologies & tools termini come schedule, release, snapshot e backup, e la funzione di punteggio non pesa il segnale contro il contesto della Capability. Insieme sono stati riportati a unclassified anche Task QM (quality management) mappato a Quality Certification per il solo token quality, Team Revisori EN mappato a Technical Knowledge Architecture per il solo token team, e Authentication Policy AllowBasicAuthIMAP-Odoo mappato a Backup & Disaster Recovery per il solo token policy. La nuova Capability proposta era Bug Quantita 0 Su Righe SO (Blocca Ritorno A Preventivo), che e' un bug specifico non una Capability: rifiutata. Rimangono quindici fit su sette Capability, pubblicati sul repo pubblico con export_to_taxonomy.py --apply.

Al verify di riservatezza dopo l'apply si sono presentati quattro riscontri della categoria nome-persona sulla parola Procedura, tutti falsi positivi. La causa era una voce nella mappa di anonimizzazione, Procedura Reso = [PERSONA_52], prodotta dal riconoscitore italiano di NER[^27] su un titolo di sezione della procedura di reso delle marche da bollo. La funzione derive_person_terms di verify_public_repo.py promuove al set dei nomi da cercare anche i singoli token di ogni voce a due parole con l'iniziale maiuscola, e cosi' Procedura e Reso sono finiti nella regex, generando quattro segnalazioni su file tecnici che parlano di procedure. Il fix e' stato aggiungere al set NON_NAMES le parole italiane comuni che il NER estrae erroneamente come nomi propri quando le trova appaiate in una coppia Nome Cognome: procedura, reso, storno, cambio, anno, ordine, fattura, fatture, sequenze, sequenza, tabella, pagina, codice, codici, magazzino, progetto, progetti, marca, marche, bollo, bolli, lavori, vendite, acquisti, revisione, traduzione, preventivo, preventivi, trasferimento, impostazioni. Il verify e' tornato pulito dopo la fix.

Restano sul tavolo due valutazioni. La prima e' se stringere le keyword della Capability Time & Attendance Management, che nel ciclo ha rivendicato otto nodi fuori tema per parole innocue del suo ## Technologies & tools; la scelta minima e' guardare quelle keyword una a una e togliere le tre che hanno prodotto la fuga. La seconda e' che sei nuovi domini sono usciti dal map_to_taxonomy.py come proposte a media coerenza, tra cui Autoassegnazione Task Revisione/Traduzione con tre nodi e Pulizia Sessione Browser (Cookies/Cache/Devtools) con sette nodi. Nessuno e' un dominio della tassonomia esistente, quindi vanno valutati manualmente se aprire una pagina nuova o se assorbire i nodi in Capability esistenti; ma non sono un debito, sono un'osservazione da riprendere quando si affronta un ciclo che tocchi la stessa area.

Numeri finali del ciclo: ventiquattro documenti in ingresso, quindici evidenze pubblicate su sette Capability, zero errori di sanitizzazione al verify dopo la fix del set dei falsi nomi. Nessuna nuova Capability creata sul repo pubblico.
...
[^27]: NER, Named Entity Recognition - riconoscitore di entita' testuali (persone, organizzazioni, luoghi) usato in enrich_graph.py; il modello italiano usato tende a scambiare per nome proprio qualsiasi coppia parola parola a iniziali maiuscole, anche quando entrambi i termini sono sostantivi comuni.
